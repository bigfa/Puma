<article class="pBlock--item<?php if (is_sticky()) echo ' sticky'; ?>" itemscope itemtype="http://schema.org/Article">
    <header class="pBlock--header">
        <h2 class="pBlock--title" itemprop="headline">
            <a href="<?php the_permalink(); ?>" title="<?php the_title(); ?>"><?php the_title(); ?></a>
        </h2>
        <div class="pBlock--postMetaWrap">
            <time datetime="<?php echo get_the_date('c'); ?>" itemprop="datePublished"><?php echo human_time_diff(get_the_time('U'), current_time('U')) . __(' ago', 'Puma'); ?></time>
        </div>
    </header>
    <div class="pBlock--snippet pGraph" itemprop="about">
        ...
        if ((has_post_thumbnail() || !$pumaSetting->get_setting('hide_home_cover')) && puma_is_has_image(get_the_ID())) : ?>
            <p class="with-img">
                <img src="<?php echo puma_get_background_image(get_the_ID()); ?>" alt="<?php the_title(); ?>" class="home--cover" itemprop="image" />
            </p>
